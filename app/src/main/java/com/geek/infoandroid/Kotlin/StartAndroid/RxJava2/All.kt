...
class All:AppCompatActivity(R.layout.main_activity2){
    fun work() {
        ...
        val flowable = Flowable.create<String>({ for (i in 1..5){it.onNext(i.toString())}},BackpressureStrategy.DROP)//тоже самое ,имеет еще backpressure
        ...
        val dispose2 = observable
            ...
            .sample(2,TimeUnit.SECONDS)//каждые 2 сек будет отправлять последний элемент,который был заэмичен
        ...
        val disp8 = observable2.count().subscribe({},{})//делает сингл,вернет количество элементов
            //defaultIfEmpty
        val dips9 = observable.defaultIfEmpty("not Found").subscribe({},{})//если пустой элемент,выведет то отправит not found
            //timeInterval
        val disp10 = observable.timeInterval().subscribe { it.time().toString() + it.value() }//получаем интервал времени между элементами
            //timeout
