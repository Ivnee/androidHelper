...
class FlowCoroutines {
    ...
    //операторы
    //intermediate (не запускает код)->  onEach, onStart, OnCompletion, onEmpty, map, filter, take, zip, combine, withIndex, scan, debounce, distinctUntilChanged, drop, sample.
    //terminal (запускает код)-> collect, single, reduce, count, first, toList, toSet, fold

    private fun flowCreation() {
        val flow = flow {//стандартный флоу
            emit("1")
            emit("2")
        ...
        val flowFunc = ::getData.asFlow()//получаем с метода инфу и делаем из этого флоу(метод ниже)
        ...
        /////////////////////channel flow
        val flowChannel = channelFlow {//может делать работу в нескольких корутинах,а результат отправит в основной поток(для паралельной работы)
            launch {
                delay(1000)
                send(1)
            }
