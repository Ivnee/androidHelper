...
import com.geek.infoandroid.LiveDataViewModel.KOTLINViewModel.MainViewModel
import com.geek.infoandroid.R

class MainFragment : Fragment(R.layout.fragment1) {

    companion object {//создаем фрагмент здесь
...
и делаем для включения вью модели
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val str = "10"
        viewModel = ViewModelProvider(requireActivity(),MainViewModelFactory(str)).get(MainViewModel::class.java)//создаем вью модель которой передали фэктори
    }
}
class MainViewModelFactory(val resourceProvider: String) : ViewModelProvider.Factory {//фектори для вью модели
    override fun <T : ViewModel?> create(modelClass: Class<T>): T = MainViewModel(resourceProvider) as T
