...
class Scope : AppCompatActivity(R.layout.activity_main) {
    ...
    private fun onRun() {
        scope.launch {
            ...
            job.join()//приостановить код пока корутина не завершит свою раюоту(в данном случае ожидает завершения чаилд корутины)приостановит выполнение родительской корутины,но не заблокирует ее поток
        }
...
//вложенный корутин
    ...
    private fun ExceptionsScope() {
        ...
        scope.launch {
            ...
            launch(SupervisorJob(coroutineContext[Job])) {}//костыль который сработает для остановки эксепшена(но лучше SupervisorScope())
    ...
    private fun coroutineChannel() {
        ...
        val channel2 =
            Channel<Int>(Channel.Factory.CONFLATED)//хранится одно значение в буфере, каждый сенд н ждет получается ,а просто перезаписывает значение
