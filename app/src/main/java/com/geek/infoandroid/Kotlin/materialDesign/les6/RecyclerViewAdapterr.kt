...
class RecyclerViewAdapterr(
    ...
    inner class MarsViewHolder(itemView: View) : BaseViewHolder(itemView),
        ...
        override fun onItemSelected() {
            itemView.setBackgroundColor(Color.BLUE)//цвет во время свайра или драга
        }

        override fun onItemClear() {
            itemView.setBackgroundColor(0)//цвет после свайпа или драга
        }
...
class ItemTouchHelperCallback(private val adapter: RecyclerViewAdapterr) :
    ItemTouchHelper.Callback() {//коллбэй для свайпов и для драгов

    override fun isLongPressDragEnabled(): Boolean {//включаем драг по долгому нажатию
        return true
    }


    override fun isItemViewSwipeEnabled(): Boolean {//включаем свайп
        return true
    }

    override fun getMovementFlags(//как будут работать наши тачи хелперы
        recyclerView: RecyclerView,
        viewHolder: RecyclerView.ViewHolder
    ): Int {
        val dragFlag =
            ItemTouchHelper.UP or ItemTouchHelper.DOWN//передвигание сраюатывает вверх и вниз
        val swipeFlag =
            ItemTouchHelper.START or ItemTouchHelper.END//свайп срабатываем слева и справа
        return makeMovementFlags(dragFlag, swipeFlag)
    }

    override fun onMove(//когда меняем элементы местами
        recyclerView: RecyclerView,
        viewHolder: RecyclerView.ViewHolder,
    ...
    override fun onSwiped(
        viewHolder: RecyclerView.ViewHolder,
        direction: Int
    ) {//когда смахиваем влево или вправо
    ...
    override fun onSelectedChanged(
        viewHolder: RecyclerView.ViewHolder?,
        actionState: Int
    ) {//когда нажимаем итем вью холдера
    ...
    override fun clearView(
        recyclerView: RecyclerView,
        viewHolder: RecyclerView.ViewHolder
    ) {//метод вызывается когда отпускаем итем холдера
...
interface ItemTouchHelperAdapter {
    //интерфейс адаптера для итем тач хелпера
    fun onItemMove(fromPosition: Int, toPosition: Int)
    fun onItemDismiss(position: Int)
}

interface ItemTouchHelperViewHolder {
    //коллбэк вью холдера для итем тач хелпера
    fun onItemSelected()
    fun onItemClear()
}
