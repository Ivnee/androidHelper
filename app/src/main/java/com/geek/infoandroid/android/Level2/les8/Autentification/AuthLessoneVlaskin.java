package com.geek.infoandroid.android.Level2.les8.Autentification;
...
public class AuthLessoneVlaskin extends AppCompatActivity {
    ...
    protected void onStart() {
        ...
        GoogleSignInAccount account = GoogleSignIn.getLastSignedInAccount(this);//здесь по сути проверка аккаунта на бан,если был забанен то облом
        if (account != null) {
    ...
    private void handleSignInResult(Task<GoogleSignInAccount> completedTask) {
        try {
            GoogleSignInAccount account = completedTask.getResult(ApiException.class);//передаем класс апи эксепт чтоб нам вернулась ошибка которую в кэтче обработает

            // Регистрация прошла успешно
            disableSign();
            updateUI(account.getEmail());//получаем почту пользоваетля
    ...
    private void updateUI(String e_mail) {
        TextView email = findViewById(R.id.button);
